...
The completion architecture relies on the https://github.com/cdrage/complete[`cdrage/complete`] project.
`cdrage/complete` relies on providing implementations of their `Predictor` interface:
...
`completion.RegisterCommandFlagHandler` respectively. These functions will adapt the contextualized predictor that we expose to commands to the `cdrage/complete` `Predictor` interface internally.
...
While filtering is done by `cdrage/complete` itself, automatically removing all completions not prefixed by what you’ve typed already, it might be useful to use the values provided by `parsedArgs` to help optimize things to avoid repeating possible completions for example.
