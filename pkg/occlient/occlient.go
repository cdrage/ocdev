...
func (c *Client) NewAppS2I(commonObjectMeta metav1.ObjectMeta, builderImage string, gitUrl string, inputPorts []string) error {
	...
	if err != nil {
		return errors.Wrap(err, "unable to parse image commonObjectMeta.Name")
	}
...
func (c *Client) UpdateDCToSupervisor(commonObjectMeta metav1.ObjectMeta, componentImageType string) error {
	...
	if err != nil {
		return errors.Wrap(err, "unable to parse image name for DeplotmentConfig update")
	}
...
/*
// UpdateDCImage updates the DeploymentConfig image that is being used in order to go from:
// local / binary -> Git
// or
// git -> local / binary
func (c *Client) UpdateDCImage(commonObjectMeta metav1.ObjectMeta, imageName string) error {

	// Retrieve the DC
	dc, err := c.GetDeploymentConfigFromName(commonObjectMeta.Name, c.namespace)
	if err != nil {
		return errors.Wrapf(err, "unable to get DeploymentConfig %s", commonObjectMeta.Name)
	}
	...
	// Iterate through the containers to find the one with the correct commonObjectMeta.Name
	foundContainer, err := findContainer(&dc.Spec.Template.Spec.Containers, commonObjectMeta.Name)
	if err != nil {
		return errors.Wrapf(err, "Unable to find containers %s", commonObjectMeta.Name)
	}
	foundContainer.Image = imageName

	// Update the triggers as well.
	...
	if err != nil {
		return errors.Wrapf(err, "unable to update DeploymentConfig config %s", commonObjectMeta.Name)
	}
	return nil
}
*/
