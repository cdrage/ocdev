...
func (wo *WatchOptions) Complete(name string, cmd *cobra.Command, args []string) (err error) {
	...
	conf, err := utils.RetrieveLocalConfigInfo(wo.componentContext)
	if err != nil {
		return errors.Wrap(err, "unable to retrieve config information")
	}
	wo.localConfig = conf.LocalConfig
	wo.sourcePath = conf.SourcePath
	wo.sourceType = conf.LocalConfig.GetSourceType()

	// Apply ignore information
	err = utils.ApplyIgnore(&wo.ignores, wo.sourcePath)
	if err != nil {
...
func (wo *WatchOptions) Validate() (err error) {
	...
	if wo.sourceType != "binary" && wo.sourceType != "local" {
		return fmt.Errorf("Watch is supported by binary and local components only and source type of component %s is %s", wo.localConfig.GetName(), wo.sourceType)
	}
